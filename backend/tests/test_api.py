...
def test_student_signup_and_login(client, db_session):
    ...
    response = client.post("/api/auth/verify-otp", json=verify_data)
    assert response.status_code == 200
...
def test_exam_lifecycle(client, db_session):
    ...
    exam_data = {
        ...
        "exam_sections": [{"title": "Section A", "questions": []}]
    }
    create_response = client.post("/api/exams", json=exam_data, headers=headers)
    ...
    assign_response = client.post(f"/api/exams/{exam_id}/assign/section", json=assign_data, headers=headers)
    ...
    exams_response = client.get("/api/users/me/exams", headers=student_headers)
    ...
    delete_response = client.delete(f"/api/exams/{exam_id}", headers=headers)
